...
export default function AllRequestsPage() {
  ...
  const params = {
    ...
    ...(searchTerm   && { searchColumn: 'studentNameAr,studentNameEn,universityCode,formTitleAr,formTitleEn', searchTerm }),
    pageNumber,
    pageSize: PAGE_SIZE,
  }

  const { data: paged, isLoading, isError, isFetching, refetch } = useAllRequests(params)
  ...
  return (
    <div dir={dir} className="space-y-6">
      ...
      {!isLoading && !isError && requests.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {requests.map(r => (
            <RequestCard key={r.id} request={r} onView={setSelectedRequest} showStudent={true} />
          ))}
        </div>
      )}
