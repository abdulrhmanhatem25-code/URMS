...
import { useCreateForm } from '../hooks/useAdminForms'
...
export default function CreateFormModal({ isOpen, onClose }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      description: '',
      isActive: true,
      startDate: toDateLocal(new Date()),
      endDate: toDateLocal(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)), // default +7 days
    },
  })
  ...
  const handleAddField = () => {
    ...
    setExpandedId(newFieldId) // expand the new field automatically
  }
  ...
  // Delete a field from local state
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={lang === 'ar' ? 'إنشاء نموذج جديد' : 'Create New Form'}
      dir={dir}
      size="xl"
    >
          ...
          <div className="p-6 space-y-3 min-h-[300px]">
            {fields.length === 0 && (
              <div className="flex flex-col items-center justify-center h-40 text-sm text-muted-foreground gap-3">
                <p>{lang === 'ar' ? 'لم تقم بإضافة أي حقول بعد' : 'No fields added yet'}</p>
                  ...
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-sm font-medium">
                        {lang === 'ar' ? (field.labelAr || 'حقل جديد') : (field.labelEn || 'New Field')}
                      </span>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border bg-secondary/20 shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <Button type="submit" disabled={isPending} className="gap-2">
              ...
              {lang === 'ar' ? 'إنشاء النموذج' : 'Create Form'}
            </Button>
          </div>
        </div>
      </form>
